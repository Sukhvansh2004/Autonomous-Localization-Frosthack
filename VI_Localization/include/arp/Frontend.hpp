...
namespace arp
{
  ...
  public:
    ...
    Frontend(int imageWidth, int imageHeight, double focalLengthU,
             ...
             double p2, int uniformityRadius, int octaves, float absoluteThreshold, int maxNumKpt, float mappingCamFocalLength);
  ...
  /// \brief Assign DBoW2 histogram to map
  /// \return True on success.
  bool assignDBoW2Histograms();
  ...
  protected:
    ...
    bool ransac(const std::vector<cv::Point3d> &worldPoints, const std::vector<cv::Point2d> &imagePoints, kinematics::Transformation &T_CW, std::vector<int> &inliers, bool requireRansac) const;
    ...
    std::vector<uint64_t> dBowDatabaseIndicesToPoseID;
    uint64_t activeKeyframe;
    std::vector<uint64_t> candidateKeyframes;
 private:
  Frontend() = delete;
};
